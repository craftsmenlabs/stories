...
public class StoryScorer {
    ...
    public static UserStoryValidatorEntry performScorer(String userStory, ValidationConfig validationConfig) {
        ...
        if (userStory == null || userStory.isEmpty())
        {
            ...
            List<String> asKeywords = validationConfig.getStory().getAsKeywords();
            if (asKeywords != null && asKeywords.stream().anyMatch(s -> userStoryLower.contains(s.toLowerCase()))) {
                points += 0.2f;
            } else {
            ...
            List<String> iKeywords = validationConfig.getStory().getIKeywords();
            if (iKeywords != null && iKeywords.stream().anyMatch(s -> userStoryLower.contains(s.toLowerCase()))) {
                points += 0.2f;
            } else {
            ...
            List<String> soKeywords = validationConfig.getStory().getSoKeywords();
            if (soKeywords != null && soKeywords.stream().anyMatch(s -> userStoryLower.contains(s.toLowerCase()))) {
                points += 0.4f;
            } else {
